...
정리 기준: 2026-09-14 기준 `main` (커밋 `0dcaa63`)의 실제 코드(`index.html`, `app.js` 5,965줄)를 읽고, **배포되는 그 파일을 브라우저로 직접 실행해 동작까지 확인**한 결과

> 확인 방법: 배포 주소(`kittycong.github.io`)는 작업 환경의 네트워크 정책에서 차단돼 직접 열지 못했습니다.
> 대신 GitHub Pages가 배포하는 `outputs/recruitment_calendar_app` 폴더를 그대로 로컬에서 띄워 확인했고,
> 로컬 `HEAD`와 `origin/main`이 모두 `0dcaa63`으로 같은 것을 확인했으므로 **배포본과 동일한 코드**입니다.
...
## 3. 실제 동작 검증 (브라우저 실행 결과)

앱을 실제로 띄워 값을 넣어 보고 확인한 결과입니다. 문서의 계산 규칙이 코드 주석이 아니라 **실제 화면 출력**과 일치합니다.

**일정 자동계산 — 공고일 2026-09-14 입력**

| 조건 | 접수기간 | 공휴일 보정 | 서류심사 | 2차 면접(예정) | 적격여부 |
|---|---|---|---|---|---|
| 일반 공고 | 9/14 ~ **10/1** | 추석 2일 반영(9/24·9/25) | 10/2(금) | **10/6(화)** | 10/7 ~ 10/8 |
| 긴급 공고 | 9/14 ~ **9/22** | 없음 | 9/23(수) | **9/29(화)** | 9/30 ~ 10/1 |
| 일반 · 9/20 공고 | 9/20 ~ **10/8** | 추석 2일 + 개천절 대체 1일 = **3일** | 10/12(월) | 10/14(수) | 10/15 ~ 10/16 |

- 일반 공고에서 면접이 10/5(월)을 건너뛴 것은 **개천절 대체공휴일**이기 때문입니다.
- 긴급 공고에서 면접이 9/28(월)이 아니라 9/29(화)로 잡힌 것은 **임원 캘린더에 9/28 김은서 일정**이 있어서입니다. 임원 회피 로직이 실제로 작동하는 것을 확인했습니다.
- 현재 임원 캘린더 동기화 상태: 2026-09-14 07:14 갱신, 등록 임원 4명, 바쁨 날짜 20일(9/9 ~ 12/22).

**수습 자동계산 — 입사일 2026-09-01 입력**
→ 수습끝 `2026-11-30`, 근무평가 작성일 `2026-10-30`, 등급 `D`(평점 미입력). 문서의 계산식과 일치합니다.

**화면에 실제로 뜬 운영 현황(기본 내장 데이터 기준)**

- 채용건 8건 / 칸반: 면접대기 8명 · 채용 3명 · 불합격 9명 (지원자 20명)
- 수습자 6명, 채용자 목록 3명 자동 집계
- 오늘 할 일에 `수습종료 · 유완정 수습평가`가 정상 표시(수습종료일 2026-09-14)

**확인된 불일치 1건 — 상단 “수습중 / 종료임박” 지표가 항상 0**
상단 요약의 `수습중`·`종료임박`은 **채용건의 채용일자**를 기준으로 계산하는데, 실제 수습자는 **수습관리 탭의 별도 명단**에 들어 있습니다. 두 데이터가 연결돼 있지 않아 수습자가 6명 등록돼 있어도 화면에는 `수습중 0`으로 나옵니다. 수습 현황은 상단 지표 대신 **수습관리 탭에서 확인**해야 합니다. (`renderSummary`가 `state.candidates`만 세고 `state.probations`를 세지 않습니다.)

---

## 4. 앱에 내장된 실데이터 — 개인정보 확인 필요

앱을 처음 여는 브라우저에서도 채용건 8건 · 지원자 20명 · 수습자 6명이 이미 보입니다. 이건 저장된 데이터가 아니라 **`app.js` 코드 안에 하드코딩된 기본 데이터**이고, 저장소가 public이므로 **누구나 소스에서 볼 수 있습니다.**

| 내장 데이터 | 위치 | 포함 내용 |
|---|---|---|
| 채용건 8건 + 지원자 15명 | `REGISTERED_RECRUITMENTS` (app.js) | 공고명·시행번호·일정, 지원자 이름, 연락처 뒤 4자리, 합격/불합격 상태 |
| 수습자 6명 | `loadProbations()` 기본값 | **실명** + 부서 + 입사일 + 평점 13개 + 합계점수 + `수습종료(퇴사)`·`육아휴직대체` 비고 |
| 임원 4명 | `exec-busy.json` | 이름 + 날짜별 바쁨 여부(일정 제목은 저장 안 함) |

- 지원자 15명 중 **6명만 마스킹**(`이*민`)돼 있고 **9명은 실명**입니다.
- 수습자 6명은 마스킹 없이 실명이며, **인사평가 점수와 퇴사 사유까지** 들어 있습니다.

> **권고**: ① 저장소를 private으로 바꾸거나, ② 시드 데이터를 마스킹된 예시(`홍*동`, 가상 점수)로 교체하세요. 코드에서 지워도 git 이력에는 남으므로, 민감하다고 판단되면 이력 정리까지 필요합니다. 앞으로도 실제 지원자 데이터는 커밋하지 말고 브라우저 입력 + 백업 JSON(로컬 보관)으로만 다루는 것이 안전합니다.
...
## 7. 계획 문서(`recruitment_probation_workflow.md`) 대비 구현 현황
...
5. **저장소가 public인데 실제 인사 데이터가 코드에 들어 있습니다.** 브라우저에 입력한 내용은 저장소에 올라가지 않지만, 기본 내장 데이터(지원자 15명·수습자 6명)는 `app.js`에 하드코딩되어 이미 공개돼 있습니다(4장 참조). 우선순위 높은 정리 대상입니다. 앞으로도 백업 JSON이나 이력서 파일을 커밋하지 마세요.
6. **상단 "수습중" 지표는 0으로 고정입니다.** 수습 현황은 수습관리 탭에서 확인하세요(3장 불일치 항목).

---

## 9. 다음에 손대면 효과 큰 순서 (제안)

1. **내장 실데이터 정리(개인정보)** — 시드 데이터를 마스킹 예시로 교체하거나 저장소를 private으로. 기능 개선보다 먼저 볼 일입니다.
2. **입사지원현황표를 앱 안으로** — 지원자 CSV 가져오기 + XLSX 내보내기. 지금 가장 손이 많이 가는 구간(③)입니다.
3. **수습명단 XLSX 다운로드** — 이미 데이터는 다 있고 내보내기만 없습니다.
4. **신규입사 이름표 HWPX** — 채용자 목록 데이터로 바로 생성 가능(양식만 준비되면 반나절 작업).
5. **팀장 서류검토 의견란** — 지원자별 의견/심사결과 필드 추가(현재는 메모 한 칸).
6. **공휴일 표 연도 자동 확장 또는 공공 API 연동** — 매년 수동 수정 부담 제거.
7. (중장기) **서버 저장소 도입** — 다중 사용자 공유와 데이터 유실 위험 해소. 현재 구조의 가장 큰 제약입니다.
